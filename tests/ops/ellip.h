...
#ifndef EXOPLANET_ELLIP_EPS
#define EXOPLANET_ELLIP_EPS (2.220446049250313e-16)
#endif
...
*/
template <typename T>
inline T _cel(T ksq, T kc, T p, T a, T b) {
  ...
  if ((ksq == 1) || (kc == 0)) kc = EXOPLANET_ELLIP_EPS * ksq;
  ...
  if (ksq > 1) ksq = 1;
  T ca = sqrt(EXOPLANET_ELLIP_EPS * ksq);
...
// Computes the function `cel(kc, p, a, b)` from Bulirsch (1969)
template <typename T>
inline T cel(T ksq, T p, T a, T b) {
  T kc;
  // Avoid undefined k2=1 case:
  if (ksq != 1.0)
    kc = sqrt(1.0 - ksq);
  else
    kc = EXOPLANET_ELLIP_EPS * ksq;
  return _cel(ksq, kc, p, a, b);
}
...
template <typename T>
inline void cel_multi(T k2, T kc, T p, T a1, T a2, T a3, T b1, T b2, T b3, T &Piofk, T &Eofk,
                      T &Em1mKdm) {
  // Bounds checks
  if (k2 > 1)
    k2 = 1;
  else if ((k2 == 1.0) || (kc == 0.0))
    kc = EXOPLANET_ELLIP_EPS * k2;
  else if (k2 < EXOPLANET_ELLIP_EPS)
    k2 = EXOPLANET_ELLIP_EPS;

  // Tolerance
  const T ca = sqrt(EXOPLANET_ELLIP_EPS * k2);
