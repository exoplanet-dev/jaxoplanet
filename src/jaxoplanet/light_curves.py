...
from jaxoplanet.core.limb_dark import light_curve as limb_dark_light_curve
from jaxoplanet.core.quad import light_curve as quad_light_curve
...
class LimbDarkLightCurve(NamedTuple):
    ...
    def light_curve(
        ...
        *,
        ...
        lc_func = partial(limb_dark_light_curve, self.u, order=order)
        if orbit.shape == ():
            b /= r_star
            lc = lc_func(b, r)
        else:
...
class QuadLightCurve(NamedTuple):
    ...
    def light_curve(
        ...
        *,
        ...
        b = jnp.sqrt(x**2 + y**2)
        r = orbit.radius / r_star
        lc_func = partial(quad_light_curve, self.u1, self.u2, order=order)
        if orbit.shape == ():
            b /= r_star
            lc = lc_func(b, r)
        else:
